...
use crate::dke_core::state_augmentation::{ augment_state_solve, augment_state_write };
...
pub struct SimulationSummary {
  ...
  pub exec_time_per_step_ms: f64,
  pub sim_to_real_ratio: f64,
...
impl DKE {
  ...
  pub fn set_param_sim_archive_flush_interval_s(
    &mut self,
    param_sim_archive_flush_interval_s_in: &f64
  ) {
...
impl DKE {
  pub fn get_dt_s(&self) -> f64 {
    self.dt_s
  }
  pub fn get_sim_time_s(&self) -> f64 {
...
impl DKE {
  pub fn run_simulation(&mut self, verbose: bool) -> SimulationSummary {
    ...
    let num_steps: i64 = (((self.sim_end_time_s - self.sim_start_time_s) / self.dt_s) as i64) + 1;
    ...
    for sim_step in tqdm(0..num_steps).style(tqdm::Style::Block) {
        ...
        sim_step == num_steps - 1
      {
        log.log_dbg(
          &format!(
            "SimTime [s] {:.3?} ( {:.2?}  {:.2?}  {:.2?} ) ->> Altitude [m] {:.2?}",
            self.sim_current_time_s,
            state_vec[STATE_VEC_INDX_POS_X],
            state_vec[STATE_VEC_INDX_POS_Y],
            state_vec[STATE_VEC_INDX_POS_Z],
            self.state.get_altitude(&state_vec)
          )
        );
      ...
      state_vec = step(&state_vec, &dxdt, self.dt_s, &mut self.environment);
      /* +_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_+_ */
      /* -------------------------------------------------------------------- */
      ...
      if write_out_counter >= self.param_sim_archive_interval_s || sim_step == num_steps - 1 {
    ...
    if verbose == true {
      ...
      log.log_msg(&format!("Number of integration steps             : {:?}", num_steps));
      log.log_msg(&format!("Step size                          [ms] : {:?}", self.dt_s * 1000.0));
      log.log_msg(
        &format!(
          "Simulation time                    [ms] : {:.3?}",
          simulation_timer.elapsed().as_millis()
        )
      );
      log.log_msg(
        &format!(
          "Exec time per step                 [ms] : {:.6?}",
          (simulation_timer.elapsed().as_millis() as f64) / (num_steps as f64)
        )
      );
      log.log_msg(
        &format!(
          "Simulated time / time to simulate   [-] : {:.1?}",
          ((self.sim_end_time_s - self.sim_start_time_s) /
            (simulation_timer.elapsed().as_millis() as f64)) *
            1000.0
        )
      );
      log.log_msg("---------------------------------------------------------------");
    ...
    // Create a simulation results summary struct
    let runtime_ms = simulation_timer.elapsed().as_millis() as f64;
    let runtime_s = runtime_ms / 1000.0;

    let summary = SimulationSummary {
      simulated_time_s: state_vec[STATE_VEC_INDX_SIM_TIME],
      runtime_s,
      num_steps,
      step_size_ms: self.dt_s * 1000.0,
      total_exec_time_ms: runtime_ms,
      exec_time_per_step_ms: runtime_ms / (num_steps as f64),
      sim_to_real_ratio: ((self.sim_end_time_s - self.sim_start_time_s) / runtime_ms) * 1000.0,
   ...
   */
  pub fn is_exit_conditions(&mut self, x_in: &Array1<f64>) -> bool {
    if x_in[STATE_VEC_INDX_ALTITUDE_PCPF_M] < 0.0 { true } else { false }
  }
}
